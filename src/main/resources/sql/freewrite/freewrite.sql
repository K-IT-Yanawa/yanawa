...
SELECT f.POST_ID, f.USER_ID
FROM TBL_FREEWRITE f
         LEFT JOIN TBL_POST p ON f.POST_ID = p.ID
WHERE p.ID IS NULL;  -- TBL_POST에 없는 POST_ID 조회

SELECT f.POST_ID, f.USER_ID
FROM TBL_FREEWRITE f
         LEFT JOIN TBL_USER u ON f.USER_ID = u.ID
WHERE u.ID IS NULL;  -- TBL_USER에 없는 USER_ID 조회

SELECT f.POST_ID, p.TYPE
FROM TBL_FREEWRITE f
         JOIN TBL_POST p ON f.POST_ID = p.ID
WHERE p.TYPE = 1;
...
VALUES (6, 'Freewrite Test Title 6', 'Freewrite Test Content 6', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1);

INSERT INTO TBL_POST (ID, POST_TITLE, POST_CONTENT, CREATED_DATE, UPDATED_DATE, TYPE)
VALUES (7, 'Freewrite Test Title 7', 'Freewrite Test Content 7', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1);

-- 추가로 필요한 POST_ID 값도 삽입합니다.
s